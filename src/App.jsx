...
const F = { body: "'Sora', sans-serif", logo: "'Raleway', sans-serif" };
...
const S = {
  ...
  h1: { fontFamily: F.body, fontWeight: 700, fontSize: 36, color: C.blue2, marginBottom: 2 },
  sub: { fontFamily: F.body, fontWeight: 400, fontSize: 18, color: C.gray, marginBottom: 20, letterSpacing: "0.06em", textTransform: "uppercase" },
  ...
  nav: { position: "absolute", bottom: 0, left: 0, right: 0, height: 56, display: "flex", alignItems: "center", justifyContent: "center", gap: 6, background: "rgba(255,255,255,0.95)", backdropFilter: "blur(12px)", borderTop: "1px solid rgba(26,82,118,0.06)", zIndex: 100 },
  dot: (a) => ({ width: a ? 24 : 9, height: 9, borderRadius: 5, background: a ? C.blue1 : "#ddd", border: "none", cursor: "pointer", transition: "all 0.3s" }),
  btn: { background: "none", border: `1.5px solid ${C.grayLight}`, borderRadius: 7, width: 34, height: 34, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: C.grayDark, fontSize: 20, transition: "all 0.2s" },
};
...
function Badge({ value }) { const s = statusOf(value); return <span style={{ display: "inline-block", padding: "3px 10px", borderRadius: 16, fontSize: 18, fontWeight: 600, background: s.bg, color: s.color }}>{s.label}</span>; }
  ...
  return (
    <div style={{ marginBottom: 14 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 5 }}>
        <span style={{ fontSize: 18, fontWeight: 500, color: C.grayDark }}>{label}</span>
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          <span style={{ fontSize: 26, fontWeight: 700, color }}><AnimNum value={value} delay={delay} run={run} /></span>
          <Badge value={value} />
        </div>
      </div>
...
function BulletList({ items, color = C.gray }) { return items.map((it, i) => (<div key={i} style={{ display: "flex", gap: 8, marginBottom: 6, alignItems: "flex-start" }}><span style={{ width: 5, height: 5, borderRadius: 3, background: color, marginTop: 7, flexShrink: 0 }} /><span style={{ fontSize: 18, lineHeight: 1.55, color: C.grayDark }}>{it}</span></div>)); }
...
function Expandable({ title, children, color = C.blue2, defaultOpen = false }) {
  ...
  return (<div style={{ marginBottom: 6 }}><button onClick={() => setOpen(!open)} style={{ width: "100%", textAlign: "left", background: "none", border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: 8, padding: "6px 0", fontFamily: F.body, fontWeight: 600, fontSize: 18, color }}><span style={{ transition: "transform 0.2s", transform: open ? "rotate(90deg)" : "rotate(0)", fontSize: 16 }}>▶</span>{title}</button>{open && <div style={{ paddingLeft: 17, paddingBottom: 4 }}>{children}</div>}</div>);
}
  ...
  return (
    <div style={{ ...S.slide(C.white), opacity: visible ? 1 : 0, transform: visible ? "none" : "translateY(30px)", alignItems: "center", justifyContent: "center", textAlign: "center" }}>
      ...
      <div>
        ...
        <h1 style={{ fontFamily: F.logo, fontWeight: 300, fontSize: 56, color: C.gray, letterSpacing: "0.14em", marginBottom: 0 }}>CUIDARTE<span style={{ fontSize: 20, verticalAlign: "super", fontWeight: 300 }}>®</span></h1>
  ...
  return (
    <div style={{ ...S.slide(), opacity: visible ? 1 : 0, transform: visible ? "none" : "translateY(30px)" }}>
      ...
      <div style={{ ...S.card, marginTop: 12, background: "#FFF8E1", borderLeft: `4px solid ${C.warn}`, padding: "10px 16px" }}>
        ...
        <span style={{ fontSize: 24, color: C.grayDark }}>Significado do trabalho é o ponto mais forte em todas as unidades (5,2–5,5). Demandas laborais é universalmente o mais baixo (3,5–4,1), exigindo ações prioritárias.</span>
      </div>
    </div>
  );
}
      ...
      <div style={{ ...S.card, marginBottom: 12 }}>
        <ResponsiveContainer width="100%" height={270}>
          <BarChart data={barData} barCategoryGap="18%" barGap={3}>
            ...
            {UNITS.map(u => (<Bar key={u.name} dataKey={u.name} fill={u.color} radius={[4, 4, 0, 0]} maxBarSize={28}>{barData.map((_, j) => <Cell key={j} fillOpacity={0.88} />)}</Bar>))}
            <Tooltip content={<CTooltip />} cursor={false} />
            <Legend wrapperStyle={{ fontSize: 22, paddingTop: 6 }} />
          </BarChart>
        </ResponsiveContainer>
      </div>
...
function S10({ visible }) {
  const risks = [
    ...
    { title: "Organização e Fluxo de Trabalho", severity: "Média-Alta", icon: "🟠", units: "Picos, Piripiri, Caxias, Floriano", desc: "Necessidade de melhor coordenação entre setores, redistribuição de tarefas e planejamento de rotas e cronogramas realistas.", sColor: C.warn },
    { title: "Condições Estruturais do Ambiente", severity: "Média", icon: "🟡", units: "Caxias, Piripiri, Picos, Floriano", desc: "Oportunidades de melhoria na organização do espaço, manutenção de equipamentos, adequação ergonômica.", sColor: "#D4A017" },
    { title: "Comunicação e Suporte Gerencial", severity: "Moderada", icon: "🟡", units: "Caxias, Floriano", desc: "Necessidade de fortalecer canais de comunicação, suporte gerencial e coordenação operacional entre setores.", sColor: "#D4A017" },
  ];
  return (
    <div style={{ ...S.slide(), opacity: visible ? 1 : 0, transform: visible ? "none" : "translateY(30px)" }}>
...
function S11({ visible }) {
  ...
  const SD = {
    ...
    "Floriano": { p: ["Monitoramento prioritário das demandas laborais: gestão de carga e ritmo operacional.", "Fortalecimento das condições do ambiente de trabalho e recursos operacionais.", "Capacitações em competências socioemocionais para lideranças.", "Espaços de escuta ativa, diálogo e promoção do autocuidado."], t: ["Gestão de demandas laborais e planejamento operacional.", "Competências socioemocionais e liderança emocional.", "Técnicas de autocuidado e gerenciamento do estresse.", "Equilíbrio entre produtividade, logística e saúde mental."], l: ["Monitoramento contínuo dos fatores psicossociais.", "Grupos de escuta mensal e reuniões de alinhamento.", "Feedbacks periódicos entre lideranças e equipes."], e: ["Organização dos recursos operacionais e materiais.", "Melhorias nas condições estruturais do ambiente.", "Coordenação eficaz entre setores de transporte e logística."] },
  };
  ...
  return (
    <div style={{ ...S.slide(), opacity: visible ? 1 : 0, transform: visible ? "none" : "translateY(30px)" }}>
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 14, flexWrap: "wrap" }}>
        {UNITS.map((unit, i) => (
          <button key={i} onClick={() => setSel(i)} style={{ fontFamily: F.body, fontWeight: sel === i ? 700 : 400, fontSize: 22, padding: "7px 16px", borderRadius: 20, border: "none", cursor: "pointer", background: sel === i ? unit.color : "transparent", color: sel === i ? "#fff" : C.gray, transition: "all 0.25s" }}>{unit.name}</button>
        ))}
      </div>
...
function S14QR({ visible }) {
  return (
    <div style={{ ...S.slide(C.white), opacity: visible ? 1 : 0, transform: visible ? "none" : "translateY(30px)", alignItems: "center", justifyContent: "center", textAlign: "center" }}>
      <Petals size={160} opacity={0.10} style={{ position: "absolute", top: 20, right: 40 }} />
      <Petals size={100} opacity={0.07} style={{ position: "absolute", bottom: 80, left: 30 }} />
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 20 }}>
        <div>
          <h2 style={{ fontWeight: 700, fontSize: 34, color: C.blue2, marginBottom: 6 }}>Participe da Enquete</h2>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <span style={{ fontFamily: F.logo, fontWeight: 300, fontSize: 20, color: C.grayLight, letterSpacing: "0.14em" }}>CUIDARTE®</span>
        </div>
      </div>
    </div>
  );
...
const SLIDES = [
  { comp: S01, title: "Capa" },
  { comp: S02, title: "1. Introdução" },
  { comp: S03, title: "Dimensões" },
  ...
  { comp: (p) => <UnitSlide {...p} unit={UNITS[4]} index={4} />, title: "2.5 Floriano" },
  { comp: S08, title: "3. Comparativo" },
  { comp: S09, title: "3. Barras" },
...
export default function App() {
  ...
  const go = useCallback((d) => setCur(c => Math.max(0, Math.min(TOTAL - 1, c + d))), []);
  useEffect(() => {
    const h = (e) => {
    ...
    <div style={{ width: "100vw", height: "100vh", overflow: "hidden", background: C.bg, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: F.body }}>
      <div style={{ width: BASE_W, height: BASE_H, transform: `scale(${scale})`, transformOrigin: "center center", position: "relative", color: C.grayDark, overflow: "hidden" }}>
        <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 3, zIndex: 101, background: "#E8EDE0" }}>
          <div style={{ height: "100%", width: `${((cur + 1) / TOTAL) * 100}%`, background: `linear-gradient(90deg, ${C.green2}, ${C.blue1})`, transition: "width 0.4s ease" }} />
        </div>
